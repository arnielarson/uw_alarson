...
class ParticleFilter():
  ...
  def __init__(self, n_particles, n_viz_particles,
    ...
    self.VIZUALIZE_PARTIVLES = False
  ...
  def visualize(self):
    ...
    if isinstance(self.inferred_pose, np.ndarray):
      ...
      if(self.pose_pub.get_num_connections() > 0):
        #rospy.loginfo("[PF] publishing /inferredpose], inferred pose x: %s, y: %s, theta: %s"%(self.inferred_pose[0], self.inferred_pose[1], self.inferred_pose[2]))
...
if __name__ == '__main__':
  ...
  while not rospy.is_shutdown(): # Keep going until we kill it
    ...
    if pf.sensor_model.do_resample: # Check if the sensor model says it's time to resample
      ...
      #rospy.loginfo("Doing resampling")
      # Resample
      ...
      pf.visualize() # Perform visualization - this also publishes the pose..
