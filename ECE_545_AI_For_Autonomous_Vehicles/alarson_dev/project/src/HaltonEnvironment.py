...
class HaltonEnvironment(object):
    ...
    def __init__(self, mapMsg, graphFile, source, target, car_width, car_length, neighbor_radius, collision_delta):
        ...
        self.source = source
        self.target = target
        self.manager = ObstacleManager(mapMsg, car_width, car_length, collision_delta) # add bad waypoints
    ...
    def set_source_and_target(self, source, target):
        self.source = source
        self.target = target # are these used anywhere??
        GraphGenerator.insert_vertices(self.graph, [source, target], self.radius)

    def add_new_target(self, target):
        GraphGenerator.insert_vertices(self.graph, [target], self.radius)

    # I'm assuming this is the x y coords? or x, y, theta?  Just the 2 coord pose
    def get_config(self, vid):
        pose = [float(a) for a in self.graph.node[str(vid)]["state"].split()]
        #rospy.loginfo("Pose in environment.get_config: [%s]"%",".join([str(x) for x in pose]))
        return pose
