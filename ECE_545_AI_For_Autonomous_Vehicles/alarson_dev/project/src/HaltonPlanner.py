...
## Class created as part of Assignment 5 - 
# 
#
class HaltonPlanner(object):
  ...
  # Note algorithm is A* graph search + checking for collisions.
  # Algorithm operates in map (pixel) coords
  # Note2 - because there is some issues with the Utils transformation code - had to add _map_to_world
  def plan(self):
  ...
  def visualize(self, plan, pre=True):
    ...
    for i in range(plan.shape[0]-1):
      startPixel = Utils._world_to_map(plan[i], self.planningEnv.manager.map_info)
      goalPixel = Utils._world_to_map(plan[i+1], self.planningEnv.manager.map_info)
