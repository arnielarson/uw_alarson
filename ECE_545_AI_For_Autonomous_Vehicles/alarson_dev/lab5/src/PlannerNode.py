...
class PlannerNode(object):
  ...
  def get_plan_cb(self, req):
    ...
    #print '[Planner Node] plan shape: '%self.cur_plan.shape
    return resp
  ...
  def source_cb(self, msg):
    ...
    self.source_updated = True
    ## Debug code with collisions
    
    print '[Planner Node] Got new source: (%s, %s)'%(self.source_pose[0], self.source_pose[1])
    map_pose = Utils.world_to_map([self.source_pose[0], self.source_pose[1],0.0], self.environment.manager.map_info)
    print '[Planner Node] Source in map coords: (%s, %s)'%(map_pose[0], map_pose[1])
    mapval = self.environment.manager.mapImageBW[map_pose[1], map_pose[0]]
    rawmapval = self.environment.manager.mapImageGS[map_pose[1], map_pose[0]]
    print '[Planner Node] Source val: %s, rawval: %s'%(mapval, rawmapval)
    valid = self.environment.manager.get_state_validity(self.source_pose)
    print '[Planner Node] Valid location: %s \n'%valid
  ...
  def update_plan(self):
    ...
    if replan:
      ...
      if self.planner.success:
        ...
        print '[Planner Node] running post process'
