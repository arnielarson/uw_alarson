...
class LineFollower:
  ...
  def compute_error(self, cur_pose):
    ...
    while len(self.plan) > 0:
      ...
      x=self.plan[i][0] - cur_pose[0]
      y=self.plan[i][1] - cur_pose[1]
      ...
      if p[0] < 0:
          #rospy.loginfo("Dropping pose from plan")
    ...
    rerror = -cur_pose[2] + self.plan[goal_idx][2]
    error = self.waypoint_weight * werror + self.rotation_weight * rerror
